...
public class GifDecoder {
	...
	public void clear() {		
		...
		while( it<frames.size() ) {
			((GifFrame) frames.get(it)).image.recycle();
			frames.remove(frames.get(it));
		}

		frames.clear();
		frames.removeAllElements();
		frames = null; // frames read from current file
		
		lastBitmap =null;
		image = null;
		in=null;
		gct = null; // global color table
		lct = null; // local color table
		act = null; // active color table
		block = null; // current data block
		// LZW decoder working arrays
		suffix = null;
		pixelStack = null;
		pixels = null;
		...
		transparency = false; // use transparent color
		blockSize = 0; // block size last graphic control extension info
		dispose = 0; // 0=no action; 1=leave in place; 2=restore to bg; 3=restore to prev
		
		delay = 0; // delay in milliseconds
		transIndex = 0; // transparent color index
		frameCount = 0;
		status = 0;
